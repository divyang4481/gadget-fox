...
namespace DotSVN.Server.Delta
{
    ...
    public class SVNRangeTree
    {
        ...
        {
            myScratchNode = new SVNRangeTreeNode(0, 0, 0);
        }
        ...
        private SVNRangeTreeNode allocateTreeNode(int offset, int limit, int target)
        {
            ...
            SVNRangeTreeNode node2 = myFreeTreeNodes;
            myFreeTreeNodes = node2.nextFree;
            node2.left = node2.right = node2.next = node2.prev = null;
            node2.offset = offset;
            node2.limit = limit;
            node2.targetOffset = target;

            // make it head of the allocated list.
            node2.nextFree = myAllocatedTreeNodes;
            myAllocatedTreeNodes = node2;
            return node2;
        }
        ...
        public virtual void disposeList(SVNRangeListNode head)
        {
        ...
        public class SVNRangeListNode
        {
            ...
            public virtual SVNRangeListNode append(SVNRangeListNode node)
            {
                next = node;
                node.prev = this;
                node.head = head;
        ...
        public virtual SVNRangeListNode buildRangeList(int offset, int limit)
        {
        ...
        private SVNRangeTreeNode myScratchNode;

        public virtual void splay(int offset)
        {
            if (myRoot == null)
            {
                return;
            ...
            myRoot = root;
            SVNDeltaCombiner.assertCondition((offset >= root.offset) || (root.left == null && root.prev == null),
                                             "assert #4");
        }

        public virtual void insert(int offset, int limit, int targetOffset)
        {
            if (myRoot == null)
            {
            ...
            {
                bool haveToInsertRange = myRoot.next == null || myRoot.limit < myRoot.next.offset ||
